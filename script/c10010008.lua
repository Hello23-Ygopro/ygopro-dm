...
function scard.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local f=aux.DMGraveFilter(scard.thfilter1)
...
function scard.thfilter2(c,code)
	return scard.thfilter1(c) and c:IsCode(code)
end
function scard.op1(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if not tc or not tc:IsRelateToEffect(e) then return end
	local g=Duel.GetMatchingGroup(aux.DMGraveFilter(scard.thfilter2),tp,LOCATION_GRAVE,0,nil,e:GetLabel())
